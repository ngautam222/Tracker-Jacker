...
const IconComponent = ({ icon, className }: { icon: string; className?: string }) => {
  const icons: Record<string, React.ReactNode> = {
    ...
    sun:     <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M7.05 16.95l-1.414 1.414m12.728 0l-1.414-1.414M7.05 7.05L5.636 5.636M12 7a5 5 0 100 10 5 5 0 000-10z" /></svg>,
  };
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="p-2 rounded-lg hover:bg-slate-700/20 text-slate-400 hover:text-white transition-colors flex-shrink-0"
                  title="Edit habit"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536M9 11.768l6.768-6.768 3.536 3.536L12.536 15.304H9v-3.536zM7 17h4.536l8.232-8.232a1 1 0 00-1.414-1.414L10.122 15.586A1 1 0 009 16v1z" />
                  </svg>
                </button>
                <button
...
export default function Dashboard() {
  ...
  const { habits, loading: habitsLoading, addHabit, deleteHabit, toggleHabitCompletion, updateHabit } = useHabits();
  ...
  const [newHabit, setNewHabit] = useState(defaultHabitForm);
  ...
  const openEditHabit = (habit: Habit) => {
    ...
    setNewHabit({
      ...
      color: habit.color,
      icon: habit.icon,
      category: (habit.category || "other") as CategoryValue,
    });
    setShowModal(true);
  };
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-800/90 backdrop-blur-xl rounded-3xl p-6 w-full max-w-md border border-slate-700/50 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-white">{editingHabit ? "Edit Habit" : "Create New Habit"}</h3>
              <button onClick={closeModal} className="p-2 rounded-lg hover:bg-slate-700/50 text-slate-400 hover:text-white transition-colors">
